...
<div class="kanban-container" id="kanbanBoard">
    ...
    <div class="kanban-column" data-status="{{ $status }}">
        ...
        <div class="column-body" id="col-{{ $status }}">
            @if(isset($tasks[$status]))
                ...
                <div class="task-card" data-id="{{ $task->id }}" data-status="{{ $task->status }}">
                    <div class="task-card-top">
                        <div class="task-title-text {{ $task->status === 'done' ? 'done-text' : '' }}"
                             ondblclick="startInlineEdit(this, {{ $task->id }})">{{ $task->title }}</div>
                        <div class="task-actions">
                            <button class="task-action-btn" onclick="openEditModal({{ $task->id }})" title="Edit">
                                <i class="fas fa-pen"></i>
                            </button>
                            <button class="task-action-btn delete" onclick="deleteTask({{ $task->id }}, this)" title="Hapus">
                    ...
                    <div class="task-footer">
                        <div class="task-assignee">
                            @if($task->assignee)
                                ...
                                <span>{{ Str::limit($task->assignee->name, 14) }}</span>
                            @else
            ...
            <div class="inline-add-form" id="add-form-{{ $status }}">
                <input type="text" placeholder="Judul task..." id="add-title-{{ $status }}" maxlength="255">
                <textarea placeholder="Deskripsi (opsional)..." id="add-desc-{{ $status }}" rows="2"></textarea>
                <div class="inline-form-row">
                    <select id="add-priority-{{ $status }}">
                ...
                <input type="date" id="add-deadline-{{ $status }}" placeholder="Deadline">
                <div class="inline-form-actions">
                    <button class="btn-inline-save" onclick="saveInlineAdd('{{ $status }}')">
                        <i class="fas fa-plus"></i> Tambah
                    </button>
                    <button class="btn-inline-cancel" onclick="cancelInlineAdd('{{ $status }}')">Batal</button>
                </div>
            </div>
        </div>
        ...
        <div style="padding: 0 0.75rem 0.75rem;">
            <button class="add-task-btn" id="add-btn-{{ $status }}" onclick="showInlineAdd('{{ $status }}')">
...
const TASK_TYPE  = @json($type);
const TASK_TYPE_ID = @json($typeId);
...
@endphp
const taskCache = @json($taskCacheData);
